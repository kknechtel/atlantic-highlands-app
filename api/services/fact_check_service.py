...
def _check_one_claim(client: anthropic.Anthropic, claim: str, doc_text: str) -> dict:
    prompt = (
    ...
    try:
        ...
        return {"verdict": "unresolved", "evidence_quote": "", "missing": [], "conflicting": []}
    return {
...
def fact_check_presentation(db: Session, sections: list) -> dict:
    ...
    for section in sections or []:
        ...
        for kind, key, label in _iter_citations(body):
            ...
            verdict = _check_one_claim(client, body[:1500], doc.extracted_text)
            v = verdict["verdict"]
