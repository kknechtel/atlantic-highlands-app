...
| `aggregate_over_bid_threshold` | Calendar-year payments to one vendor exceed the bid threshold and the body has no bid or exempt award on file for it. Runs only for bodies that have award data. |
| `split_awards` | Two or more awards from one body to one vendor, each 80–100% of the bid threshold, within 365 days. |
